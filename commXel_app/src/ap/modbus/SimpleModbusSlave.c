...
void modbusDelayMicroSec(uint32_t usec);
...
unsigned int modbus_update(unsigned int *holdingRegs)
{
  ...
  while (dxlportAvailable(_DEF_DXL1))
  {
    // The maximum number of bytes is limited to the serial buffer size of 128 bytes
    // If more bytes is received than the BUFFER_SIZE the overflow flag will be set and the 
    // serial buffer will be red untill all the data is cleared from the receive buffer.
    if (overflow) 
      dxlportRead(_DEF_DXL1);
    else
    {
      if (buffer == BUFFER_SIZE)
        overflow = 1;

      frame[buffer] = dxlportRead(_DEF_DXL1);
      buffer++;
    }
    modbusDelayMicroSec(T1_5); // inter character time out
  }
...
void sendPacket(unsigned char bufferSize)
{
  ...
  dxlportWrite(_DEF_DXL1, frame, bufferSize);

  while(!dxlportIsTxDone(_DEF_DXL1));
  
  // allow a frame delay to indicate end of transmission
  modbusDelayMicroSec(T3_5);
